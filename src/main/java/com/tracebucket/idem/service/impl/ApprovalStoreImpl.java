...
public class ApprovalStoreImpl implements ApprovalStore{
    ...
    private Boolean refreshApproval(Approval approval){
        ...
        com.tracebucket.idem.domain.Approval a = approvalRepository.findByClientAndUserAndScope(approval.getClientId(), approval.getUserId(), approval.getScope());
        if(a != null){
            ...
            a = new com.tracebucket.idem.domain.Approval();
    ...
    public List<Approval> getApprovals(String userName, String clientId) {
        List<com.tracebucket.idem.domain.Approval> approvals = approvalRepository.findByUserNameAndClientId(userName, clientId);
        List<Approval> approvals1 = null;
        if(approvals != null && approvals.size() > 0) {
            approvals1 = new ArrayList<Approval>();
            for(com.tracebucket.idem.domain.Approval approval : approvals) {
